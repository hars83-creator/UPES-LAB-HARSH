...
function PanelReadout({ label, value }) {
  return (
    <div className="rounded-md border border-slate-700 bg-slate-950/90 px-3 py-2 font-mono text-xs text-cyan-100 shadow-lab">
...
export default function LabVisualization({ experimentId, controls, current, running, time }) {
  ...
  return (
    <section className="premium-panel overflow-hidden rounded-lg shadow-lab">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
        ...
        <span className={`rounded px-2 py-1 text-xs font-black ${running ? 'bg-emerald-300/20 text-emerald-100' : 'bg-white/[0.08] text-white/50'}`}>
          {running ? 'Running' : 'Idle'}
        </span>
      </div>

      <div className="lab-grid relative min-h-[360px] p-4">
        {experimentId === 'planck' ? (
